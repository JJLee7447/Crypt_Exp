...
//将8位的明文或密钥转换为字节
Byte AES::Bit8_to_Byte(const unsigned char & bit8){
    Byte result;
    for (int i = 0; i < 8; i++){
        /**
         * result[i] = bit8 & 1;
         * bit8 >>= 1;  //error bit8 is const
         */
        result[i] = bit8 & (1 << i);
        /**
         * 代码中，1 << i 是将 1 左移 i 位，得到一个只有第 i 位是 1 的二进制数。
         * 然后，使用按位与运算符 & 将这个二进制数与 bit8 进行按位与运算，得到的结果要么是 0，要么是一个只有第 i 位是 1 的二进制数。
         * 因为在一个二进制数中，所有不是第 i 位的位都是 0，所以只有第 i 位上的值对结果有影响。
        */
    }
    return result;
}
//将16字节的明文或密钥转换为16个8位的字节
Single_Group AES::Byte16_to_GroupByte(const Vec & Byte_16){
    Single_Group result(16, Byte(8));
    for (int i = 0; i < 16; i++){
        result[i] = Bit8_to_Byte(Byte_16[i]);
    }
    return result;
}
//将16个字节的组组合成二维数组
Groups AES::Byte16_to_Matrix(Cvec_Matrix & Byte_16){
    cout<<Byte_16.size()<<endl;
    int size = Byte_16.size();
    Groups result(size, Single_Group(16, Byte(8)));
    cout<<"result "<<result.size()<<endl;
    for (int i = 0; i < size; i++){
        result[i] = Byte16_to_GroupByte(Byte_16[i]);
    }
    return result;
}
...
string AES::Byte16_to_string16( Cvec &Byte_16){
    string str;
